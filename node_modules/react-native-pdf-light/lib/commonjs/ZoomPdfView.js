...
function getViewDims(pdfSize, viewSize, resizeMode) {
  let viewWidth = viewSize.width;

  if (pdfSize.width > 0 && (pdfSize.height > pdfSize.width && viewSize.height > viewWidth || resizeMode === 'contain' && pdfSize.height > 0)) {
...
function useZoomGesture(pdfSize, viewSize, resizeMode, maxScale, hScrollRef, vScrollRef, onZoomIn, onZoomReset) {
  // Limits.
  const minScale = 1;
  const overshoot = 0.1;
  ...
  (0, _react.useEffect)(() => {
    const {
      viewWidth,
      viewHeight
    } = getViewDims(pdfSize, viewSize, resizeMode);
  ...
  }, [bufferSize, containerScale, pdfSize, resizeMode, viewSize]); // Origin point of pinch gesture.
  ...
  const gestureHandler = (0, _react.useMemo)(() => {
    return _reactNativeGestureHandler.Gesture.Pinch().runOnJS(true).withRef(gestureRef).onStart(e => focalPoint.animated.setValue({
      x: e.focalX,
      y: e.focalY
  ...
  }, [bufferSize, containerScale, contentOffset, focalPoint, hScrollRef, gestureRef, maxScale, onZoomIn, onZoomReset, pdfSize, pinchScale, resizeMode, setIsZoomed, vScrollRef, viewSize]);
  ...
  }, [bufferSize, contentOffset, containerScale, focalPoint, pdfSize, resizeMode, scale, viewSize]);
  return {
    bufferSize,
    contentOffset,
    gestureHandler,
...
function ZoomPdfView(props) {
  ...
  const {
    ...
    isZoomed,
    zoomStyle
  } = useZoomGesture(pdfSize, viewSize, resizeMode, maximumZoom ?? 2, hScrollRef, vScrollRef, onZoomIn, onZoomReset); // Combine `onLoadComplete` callback from props with logic required
  // by `ZoomPdfView`.
